...
document.addEventListener('DOMContentLoaded', function () {
  ...
  searchBtnEl.addEventListener('click', function () {
    ...
    .then(function (results) {
      ...
      // Additional code to handle data if needed
    })
...
// If there are no results, show a modal or handle the case as needed
var modal = new bootstrap.Modal(document.getElementById('exampleModal'));
modal.show();

// Reject the promise with an error message or appropriate data
  ...
  return new Promise(function (resolve, reject) {
    ...
    fetch(menuItemsURL)
        ...
        if (data.menuItems && data.menuItems.length > 0) {
          for (var i = 0; i < data.menuItems.length; i++) {
            ...
            var menuItemObject = {
              ...
              restaurant: menuItem.restaurantChain
            };
          ...
          // Resolve the promise with the menu items
          resolve(allMenuItems);
          console.log('After processing menu items loop');
        } else {
      ...
      .catch(function (error) {
        ...
        reject(error); // Propagate the error up the chain
      });
  });
}

// Use Promise.all to wait for both getRecipes and getMenuItems to complete
Promise.all([getRecipes(ingredientOrDishName), getMenuItems(ingredientOrDishName)])
  .then(function(results) {
    console.log('Inside Promise.all then block');
    var allRecipeDetails = results[0];
    var allMenuItems = results[1];

    console.log('getRecipes and getMenuItems completed successfully:', allRecipeDetails, allMenuItems);

    if (allRecipeDetails.length === 0 && allMenuItems.length === 0) {
      ...
      console.log('Redirecting to:', '/library/results2.html');

      // Redirect to results2.html
      window.location.href = 'kelevation/library/results2.html';
      console.log('After redirecting to results2.html');
    }
  })
  .catch(function(error) {
    console.log('Inside Promise.all catch block');
    console.error('Error in getRecipes or getMenuItems:', error);
  });
